...
export default function SignUp() {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
   ...
   try {
     ...
     ProgressService.initializeProgress(registerResponse.user.id);
     
     await UserService.updateUserProfile(registerResponse.user.id, {
       email,
     ...
     setErrors(prev => ({
       ...prev,
       email: error.message || 'Kayıt işlemi başarısız'
     }));
   } finally {
     setIsLoading(false);
   }
 ...
 const handleGoogleSignup = async () => {
   setIsLoading(true);
   try {
     // Google signup implementasyonu gelecek
 ...
 const handleAppleSignup = async () => {
   setIsLoading(true);
   try {
     // Apple signup implementasyonu gelecek
         ...
         {showEmailForm ? (
           ...
           <div className="space-y-4 mb-6">
             <button 
               ...
               disabled={isLoading}
               className="w-full bg-black text-white rounded-xl p-4 flex items-center justify-center gap-3 hover:bg-gray-800 transition-all"
             >
               {isLoading ? (
                 <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin"/>
               ) : (
             ...
             <button 
               ...
               disabled={isLoading}
               className="w-full bg-white border border-gray-300 text-gray-700 rounded-xl p-4 flex items-center justify-center gap-3 hover:bg-gray-50 transition-all"
             >
               {isLoading ? (
